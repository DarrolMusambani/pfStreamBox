...
Put it into your RaspberryPi, boot up and connect your Pi to the internet. Also change the default-password.
...
Install Node-RED using the provided install-script from [nodered.org](https://nodered.org/docs/getting-started/raspberrypi). This must be done as "pi"-user so type `exit` if you're currently runnig as root.
...
## Node-Red
The Node-RED Dashboard will be used to let the players control some basic A/V-settings and add additional accespoints
